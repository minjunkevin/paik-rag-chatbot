...
def answer_stream(question: str, evidence: List[Chunk], mode: str = "grounded", history: Optional[List[Dict[str,str]]] = None):
    ...
    if mode == "grounded":
        system = (
            ...
            "반드시 제공된 '근거' 안에서만 답해라.\n"
            "근거에 없는 내용은 추측하지 말고, 모르면 모른다고 말해라.\n"
        ...
        system = (
            ...
            "그래도 질문에 대해 일반 지식과 추론으로 최대한 유용하게 답해라.\n"
            "답변의 정확도가 보장되지 않을 수 있으므로, 답변 맨 뒤에 반드시 아래 문장을 붙여라:\n"
            "※ 주의: 위의 답변은 일반 지식 기반으로 작성되었으며 일부 오류가 있을 수 있습니다.\n"
            "이상한 기호(** 같은 것) 넣지 말고 자연스럽게 써라.\n"
            "출처 목록은 작성하지 말고, 답변 내용만 작성해라.\n"
        )
        user = "질문:\n" + question
